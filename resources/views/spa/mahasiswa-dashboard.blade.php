    ...
    <div class="container">
        ...
        <div class="sidebar">
            <div class="sidebar-header">
                <h3><i class="fas fa-book"></i> Akademik</h3>
        ...
        <div class="main-content">
            ...
            <div class="header">
                <div>
                    ...
                    <p style="color: #7f8c8d; margin-top: 5px;">Selamat datang di Sistem Akademik</p>
                </div>
            ...
            <div class="content-section">
                ...
                <div id="profilContent" class="loading">
            ...
            <div class="content-section" id="kelasSection">
                <h2 class="section-title">
                    <i class="fas fa-book"></i> Kelas yang Diikuti
                </h2>
            ...
            <div class="content-section" id="jadwalSection">
                <h2 class="section-title">
                    <i class="fas fa-calendar"></i> Jadwal Kelas
                </h2>
            ...
            <div class="content-section">
                <h2 class="section-title">
                    <i class="fas fa-chalkboard-user"></i> Instruktur Pengajar
                </h2>
                <div id="instrukturContent" class="loading">
                    <div class="spinner"></div>
                    <span>Memuat data...</span>
                </div>
            </div>
    ...
    <script>
        ...
        async function loadMahasiswaData() {
            try {
                ...
                document.getElementById('userNim').textContent = `NIM: ${user.id}`;
        ...
        async function loadProfil(user) {
            try {
                // Get mahasiswa data
                const mahasiswaRes = await fetch(`${API_BASE}/academic/mahasiswa`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });

                const mahasiswaData = await mahasiswaRes.json();
                const mahasiswaList = mahasiswaData.data || [];
                const mahasiswa = mahasiswaList.find(m => m.user_id === user.id);
                ...
                document.getElementById('profilContent').innerHTML = '<div class="no-data">Error loading profil</div>';
            }
        }

        async function loadKelas(userId) {
            try {
                const res = await fetch(`${API_BASE}/academic/mahasiswa/${userId}/kelas`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                ...
                let html = '';
                if (kelas.length === 0) {
                    html = '<div class="no-data">Tidak ada kelas yang diikuti</div>';
                } else {
                    html = `
        ...
        async function loadJadwal(userId) {
            try {
                const res = await fetch(`${API_BASE}/academic/mahasiswa/${userId}/jadwal`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
        ...
        async function loadInstruktur(userId) {
            try {
                // Get kelas
                const kelasRes = await fetch(`${API_BASE}/academic/mahasiswa/${userId}/kelas`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                ...
                // Collect unique instruktur
                const instrukturMap = new Map();
                kelas.forEach(k => {
                    if (k.instruktur && !instrukturMap.has(k.instruktur.id)) {
                        instrukturMap.set(k.instruktur.id, k.instruktur);
                    }
                });

                const instrukturList = Array.from(instrukturMap.values());

                let html = '';
                if (instrukturList.length === 0) {
                    html = '<div class="no-data">Tidak ada instruktur</div>';
                } else {
                    html = `
                        <div class="table-responsive">
                            <table>
                ...
                document.getElementById('instrukturContent').innerHTML = html;

            } catch (error) {
                console.error('Error loading instruktur:', error);
                document.getElementById('instrukturContent').innerHTML = '<div class="no-data">Error loading instruktur</div>';
            }
        }
